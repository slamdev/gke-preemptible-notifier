build:
	go build -o bin/gke-preemptible-notifier ./cmd/notifier

run: build
	./bin/gke-preemptible-notifier watch
...
untag:
	...
	curl --request DELETE --header "Authorization: token ${GITHUB_TOKEN}" "https://api.github.com/repos/slamdev/gke-preemptible-notifier/releases/:release_id/$(TAG)"
